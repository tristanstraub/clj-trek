...
(def basic
  (merge
   ...
   {"statement = comment | gosub | print-using | print | input | if | assignment | dim | def | mat | for | goto | goto-of | next | image | return | end"
    (fn [machine s]
      s)}

   {"print = <\"PRINT\"> <ws?> (! 'USING' (<ws> | expression | <\",\"> | <\";\">))* "
    (fn
      ([machine]
       (emit machine :print))
      ([machine & args]
   ...
   {"print-using = <'PRINT'> <ws> <'USING'> <ws> line-number (<';'> expression (<','> expression)*)?"
    (fn [machine line-number & expressions]
      (emit machine :print-using [line-number expressions]))}

   {"goto = <\"GOTO\"> <ws> line-number"
    (fn [machine n]
      (assert (number? n))
      (emit machine :goto n))}

   {"goto-of = <\"GOTO\"> <ws> expression <ws> (<'OF'> <ws> line-number (<','> line-number)*)"
    (fn
      ([machine n]
       (emit machine :goto n))
   ...
   {"expression = v-expression | m-expression | a-expression"
    (fn [machine v]
      v)}

   {"v-expression = value | <'('> expression <')'> | h-expression"
    (fn [machine v]
      v)}

   {"a-expression = expression additive-op m-expression"
    (fn [machine left op right]
      (emit machine :binary-operator left op right))}

   {"m-expression = v-expression | m-expression multiplicative-op v-expression"
    (fn
      ([machine v]
       v)
   ...
   {"h-expression = v-expression hat-op v-expression"
    (fn [machine left op right]
      (emit machine :binary-operator left op right))}
   ...
   {"bool-expression = expression <ws?> comparison-op <ws?> expression (<ws> bool-op <ws> bool-expression)?"
    (fn [machine a comparison-op b & [bool-op right]]
   ...
   {"array-ref = identifier <'['> array-indices <']'>"
    (fn [machine identifier indices]
      (emit machine :array-ref identifier indices))}

   {"array-indices = expression (<','> expression)*"
    (fn [machine & indices]
      indices)}

   {"array-def = identifier <'['> dimensions <']'>"
    (fn [machine identifier dimensions]
      (emit machine :array-ref identifier dimensions))}

   {"dimensions = integer (<','> integer)*"
    (fn [machine & dimensions]
      dimensions)}

   {"array-defs = array-def (<#','> array-ref)*"
    (fn [machine & array-defs]
      array-defs)}
   ...
   {"def = <\"DEF\" ws> identifier <'('> identifier <')' '='> expression"
   ...
   {"image = <\"IMAGE\"> <ws> formatter-list"
    (fn [machine fl]
      fl)}

   {"formatter-list = formatter (<','> formatter)*"
    (fn [machine a & bs]
      (apply emit machine :formatter-list `[~a ~@bs]))}

   {"formatter = format-count format-type | quoted-string | format-type | (format-count \"(\" formatter-list \")\")"
    (fn
      ([machine number type]
